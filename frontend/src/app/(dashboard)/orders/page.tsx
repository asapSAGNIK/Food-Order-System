...
export default function OrdersPage() {
  ...
  const renderOrderCard = (order: Order, isDraft: boolean) => (
    <div key={order.id} className={`bg-card shadow-md overflow-hidden sm:rounded-xl border-0 ${isDraft ? 'ring-2 ring-primary ring-opacity-50' : ''}`}>
      <div className={`px-6 py-6 sm:px-6 flex justify-between items-center ${isDraft ? 'bg-primary bg-opacity-5' : ''}`}>
        <div>
          <h3 className="text-xl leading-6 font-bold text-foreground">
            {isDraft ? 'Active Cart' : `Order #${order.id}`} — <span className="text-gray-700">{order.restaurant?.name}</span>
          </h3>
          <p className="mt-2 max-w-2xl text-sm text-gray-500">
            Status: <span className={`font-bold px-2 py-1 rounded inline-block ml-1 ${isDraft ? 'text-primary bg-accent-light bg-opacity-30' : order.status === 'PLACED' ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100'}`}>{order.status}</span>
          </p>
        </div>
      ...
      <div className="border-t border-gray-100">
        <dl>
          <div className="bg-background px-6 py-4 sm:px-6">
            <dt className="text-sm font-bold text-gray-600 mb-3">{isDraft ? 'Cart Items' : 'Order Items'}</dt>
  ...
  return (
    <div>
      <div className="mb-8 flex justify-between items-center bg-card p-8 rounded-xl shadow-sm">
        <div>
          <h1 className="text-4xl font-bold text-foreground tracking-tight">Orders & Carts</h1>
          <p className="mt-2 text-sm text-gray-600">Review your current carts and past orders.</p>
        </div>
      </div>

      {orders.length === 0 ? (
        <div className="bg-card p-6 rounded-xl text-center border-0 shadow-md text-gray-500 font-medium">
          No orders or carts found.
        </div>
      ) : (
        <div className="space-y-12">
          {drafts.length > 0 && (
            <section>
              <h2 className="text-2xl font-bold text-foreground mb-4">Active Carts</h2>
              <div className="space-y-6">
                {drafts.map(order => renderOrderCard(order, true))}
              </div>
            </section>
          )}
          ...
          {history.length > 0 && (
            <section>
              ...
              <div className="space-y-6">
                {history.map(order => renderOrderCard(order, false))}
              </div>
            </section>
          )}
        </div>
